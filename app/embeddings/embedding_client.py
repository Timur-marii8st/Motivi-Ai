...
class EmbeddingClient:
    """
    Client for generating embeddings via the OpenRouter API (Qwen model).
    ...
    async def _validate_dimension(self, embedding: List[float]):
        ...
        logger.info("Embedding dimension validated: {}D (model: {})", actual_dim, self.model)

    async def embed(self, text: str, task_type: str = "retrieval_document") -> List[float]:
        """
        Compute embedding for a single text prompt.
        Note: task_type is accepted for interface compatibility but not used.
        """
        try:
...
GeminiEmbeddings = EmbeddingClient
