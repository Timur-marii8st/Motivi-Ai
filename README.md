...
* `/start` - Begin onboarding (Name, Age, Timezone, Sleep Schedule).
* `/profile` - View and edit profile data or delete account.
* `/habits` - View active habits and streaks.
* `/add_habit` - Start the flow to create a new habit.
* `/connect_calendar` - Authorize Google Calendar integration.
* `/settings` - Toggle proactive messages or enable "Break Mode".
